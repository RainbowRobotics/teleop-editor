...
class RobotManager:
    ...
    def connect(self, address: str) -> bool:
        ...
        def state_cb(state: rby.RobotState_A):
            ...
            # If SDK exposes names with each power state, use them; otherwise index-order tracking
            try:
                # best effort: map by name, else index keys
                detail = {}
                for i, key in enumerate(["5v", "12v", "24v", "48v"]):
    ...
    def ensure_power_and_tools(self, timeout_s: float = 5.0) -> bool:
        ...
        if not self.power_all_on:
            if not self.robot.power_on(".*"):  # turns on 12/24/48
                return False
        ...
        # 2) set tool-flange 12V both sides (required for gripper)
    ...
    def disconnect(self):
        ...
        self.ready = False
        self.power_all_on = False
        self.tool_flange_12v = {"left": False, "right": False}
